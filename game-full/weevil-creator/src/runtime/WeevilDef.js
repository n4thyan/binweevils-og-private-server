...
export function getDefObj(defString) {
  const source = String(defString || "");
  const legacy = source.slice(0, 18);
  const result = {
    ht: legacy.substr(0, 1),
    hc: clrs1[Number(legacy.substr(1, 2))],
    bt: legacy.substr(3, 1),
    bc: clrs1[Number(legacy.substr(4, 2))],
    et: legacy.substr(6, 1),
    ec: clrs2[Number(legacy.substr(7, 2))],
    lids: legacy.substr(9, 1),
    at: legacy.substr(10, 2),
    ac: clrs1[Number(legacy.substr(12, 2))],
    lc: clrs1[Number(legacy.substr(14, 2))],
    lt: legacy.substr(16, 2),
  };

  const extension = source.slice(18);
  if (/^~[0-9A-Fa-f]{30}$/.test(extension)) {
    const rawColors = extension.slice(1);
    const colors = [0, 6, 12, 18, 24].map((offset) => Number.parseInt(rawColors.slice(offset, offset + 6), 16));
    [result.hc, result.bc, result.ec, result.ac, result.lc] = colors;
  }
  return result;
}
