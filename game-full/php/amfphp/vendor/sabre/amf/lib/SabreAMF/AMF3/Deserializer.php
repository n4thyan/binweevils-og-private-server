<?php
    ...
    class SabreAMF_AMF3_Deserializer extends SabreAMF_Deserializer {
        ...
        public function readAMFData($settype = null) {
           ...
           if($this->target == "MovieStarPlanet.WebService.MovieService.AMFMovieService.SaveMovieWithSnapshot") {
                //$this->log = true;
            }
            if($this->ticketRead) {
                $this->storedClasses[0] = array();
                $this->storedStrings[] = "annyAttribute";
                $this->storedStrings[] = "Ticket";
                $this->storedStrings[] = "the ticket value itself";
            }
           switch ($settype) {
        ...
        public function readObject() {
            ...
            if ($storedObject) {
                ...
                if ($storedClass) {
                    if($this->log) {
                        var_dump($this->storedClasses);
                        var_dump("Trying to get " . $objInfo);
                    }
                    ...
                    if (!isset($this->storedClasses[$classReference])) {
                        ...
                        if($this->log)  {
                            echo "Property names: ".implode(",", $propertyNames)." \n";
                            echo "Stored Class name: ".$className . "\n";
                        }
                   }
                    ...
                    if($this->log) echo "Class name: ".$className . "\n";
                    $encodingType = $objInfo & 0x03;
                    $propertyNames = array();
                    $objInfo = $objInfo >> 2;
                    if($this->log) var_dump("Increasing nested class offset");
                ...
                if ($encodingType & SabreAMF_AMF3_Const::ET_EXTERNALIZED) {
                    if($this->log) var_dump("ET_EXTERNALIZED");
                    if (!$storedClass) {
                    ...
                    if ($encodingType & SabreAMF_AMF3_Const::ET_SERIAL) {
                        ...
                        if (!$storedClass) {
                            ...
                            if($this->log) var_dump($propertyCount);
                        ...
                        foreach($propertyNames as $propertyName) {
                            if($this->log) var_dump("trying to read for " . $propertyName);
                            $properties[$propertyName] = $this->readAMFData();
                            if($this->log) var_dump("------");
                        }
        ...
        public function readArray() {
            ...
            while($key!="") {
                if($this->log) var_dump("KEY SHIT");
                $data[$key] = $this->readAMFData();
                $key = $this->readString();
            }

            for($i=0;$i<$arrId;$i++) {
                if($this->log) var_dump("NORMAL");
        ...
        public function readString() {
            ...
            if (($strref & 0x01) == 0) {
                ...
                if($this->log) {
                    var_dump("Looking for " . $strref);
                    var_dump($this->storedStrings);
                }
                /*if($this->nestedClassOffset > 0) {
                    var_dump("GAVE " . $this->storedStrings[$strref - 3]);
                    return $this->storedStrings[$strref - 3];
